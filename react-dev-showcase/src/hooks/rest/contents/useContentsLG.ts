...
const useContentsLG = () => {
  ...
  const queryParams = getFilteredQueryParams(['subFilter']);
  ...
  const { data: listData } = useQuery<CPContentListResponse, Error>({
    queryKey: ['contentsRoutineLG', queryParamsObject],
    queryFn: async () => {
      const response = await Api.get<CPContentListResponse>(`/v1/admin/program/lg/contents/routine/list`, {
  ...
  const { data: itemData } = useQuery<FormValues, Error>({
    ...
    queryFn: async () => {
      const response = await Api.get<FormValues>(`/v1/admin/program/lg/contents/routine/${id}`);

      return response.data;
    },
    enabled: hasId, // NOTE: routine컨텐츠인경우 조건추가
  });
